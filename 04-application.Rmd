...
to make non-mandatory tours is also substantially diminished. More notable than
...
Though this is only `r n_changed / n_total * 100`% of individuals, it is worth
...
For example, `r change1*100`% of the wheelchair users who were modeled as 
...
`r change2 * 100` of the household members who stay at home in the new scenario
incorporating wheelchair use previously made out-of-home tours in the base scenario.


```{r dap-map, fig.cap="Percent of households with changed daily activity pattern between two scenarios.", fig.height = 8}
tar_load(dap_map)

ggplot(dap_map %>% filter(pct > 0), aes(fill = pct)) +
